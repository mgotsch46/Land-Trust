...
// ─── PAGE SETUP ───────────────────────────────────────────────────────────────
// Exact margins measured from uploaded template docx files
// Deed to Trustee: 0.75" all sides
// Special Warranty Deed: 0.5" top, 0.75" all others
// 1 inch = 1440 twips
...
function buildSpecialWarrantyDeed(d) {
  // Exact formatting from Special_Warranty_Deed.docx template:
  // - 0.5" top margin, 0.75" all others
  // - 1.15 line spacing, 0pt before/after on all paragraphs
  // - 13 blank lines for recorder space (indices 1-13 in template)
  // - 4 blank lines between return address and title (indices 18-21)
  // - Page break inside "Grantor Signatures:" paragraph (index 43)
  // - "Page 1 of 2" on same line before page break, "page 2 of 2" at end
  // - Two grantor sig lines side by side on page 2
  // - Blank lines used as spacers throughout

  const children = [
    // Recorder space — "THIS SPACE..." then 13 blank lines
    ...
    p(d.returnAddr1),
    p(d.returnAddr2),
    // 4 blank lines before title (matches template indices 18-21)
    pb(), pb(), pb(), pb(),
    // Title — not bold/centered in template, just plain caps
    p('SPECIAL WARRANTY DEED'),
    // Body
    p('THE GRANTOR(S),'),
    ...
    pb(),
    // "Page 1 of 2" then "Grantor Signatures:" with page break embedded
    p('Page 1 of 2'),
    // Page break inside Grantor Signatures paragraph — matches template exactly
    new Paragraph({
      spacing: { before: 0, after: 0, line: 276, lineRule: 'auto' },
      children: [
        new TextRun({ text: 'Grantor Signatures: ', size: 24, font: 'Times New Roman' }),
        new PageBreak(),
      ]
    }),
    // Page 2
    pb(),
    pb(),
    // Two signature lines side by side — matches template index 46-47
    new Paragraph({
      spacing: { before: 0, after: 0, line: 276, lineRule: 'auto' },
      children: [
        new TextRun({ text: '__________________________________   __________________________', size: 24, font: 'Times New Roman' }),
      ]
    }),
    new Paragraph({
      spacing: { before: 0, after: 0, line: 276, lineRule: 'auto' },
  ...
  return new Document({
    sections: [{ properties: PAGE_SWD, children }]
  });
}

function buildDeedToTrustee(d) {
  // Exact formatting from Deed_to_Trustee.docx template:
  // - 0.75" all margins
  // - 1.15 line spacing, 0pt before/after all paragraphs
  // - Empty paragraphs as spacers
  // - Page 1: header through homestead waiver paragraph (flows naturally)
  // - Page 2: blank lines + two grantor sig lines + Exempt/Section 4 lines
  //   Page break AFTER "Section 4, Real Estate Transfer Tax Act." (end of page 2)
  // - Page 3: Date, Seller, IN WITNESS WHEREOF, notary block
  ...
  const children = [
    ...
    pb(),
    pb(),
    // Signature lines — two grantor names side by side (matches template index 33-34)
    new Paragraph({
      spacing: { before: 0, after: 0, line: 276, lineRule: 'auto' },
      children: [
        new TextRun({ text: '_________________________________        _________________________________', size: 24, font: 'Times New Roman' }),
      ]
    }),
    new Paragraph({
      spacing: { before: 0, after: 0, line: 276, lineRule: 'auto' },
    ...
    pb(),
    p('Exempt under provisions of paragraph E', { bold: true }),
    // Page break AFTER Section 4 line — this ends page 2
    new Paragraph({
      spacing: { before: 0, after: 0, line: 276, lineRule: 'auto' },
      children: [
        new TextRun({ text: 'Section 4, Real Estate Transfer Tax Act.', size: 24, font: 'Times New Roman', bold: true }),
        new PageBreak(),
      ]
    }),
    // Page 3 begins here
    p(`Date: ${d.saleDate ? fmtDateShort(d.saleDate) : fmtDateShort(d.trustDate)}`, { bold: true }),
    p(`Seller: ${d.grantor}`, { bold: true }),
    pb(),
    pb(),
    pb(),
  ...
  return new Document({ sections: [{ properties: PAGE_DTT, children }] });
}
